...
* **On-Device S1-mini Dictation Cleanup**: Optional local text normalization powered by Superwhisper's [s1-mini](https://huggingface.co/superwhisper/s1-mini-GGUF) (fine-tuned Qwen3-0.6B quantized model, ~480 MB download). Running 100% on-device in pure Rust via Candle, S1-mini transforms raw speech-to-text transcripts into polished prose with natural punctuation and spoken self-corrections while strictly preserving voice commands and trigger keywords. Enable it in Settings → Engine, with per-keybind toggles available in Hotkey settings.
