//! VoxCPM2 neural text-to-speech engine support (pure Rust / Candle).
//!
...
//! (reference .wav clips), including Ultimate Cloning with paired audio + transcript.
...
const VOX_CPM_2_REPO: &str = "openbmb/VoxCPM2";
...
/// Download VoxCPM2 assets from HuggingFace into model_dir
pub async fn download_vox_cpm_2_assets(model_dir: &str, hf_token: Option<String>) -> Result<()> {
    ...
    for file in files_to_fetch {
        ...
        let url = format!("https://huggingface.co/{VOX_CPM_2_REPO}/resolve/main/{file}");
        ...
        let resp = request
            .send()
            .await
            .with_context(|| format!("fetch {file} from {VOX_CPM_2_REPO}"))?;

        let status = resp.status();
        if status == reqwest::StatusCode::UNAUTHORIZED || status == reqwest::StatusCode::FORBIDDEN {
            return Err(crate::hf::token_rejected(VOX_CPM_2_REPO));
        }
        if !status.is_success() {
            anyhow::bail!("fetch {file} from {VOX_CPM_2_REPO}: HTTP {status}");
        }

        let bytes = resp
            .bytes()
            .await
            .with_context(|| format!("read {file} from {VOX_CPM_2_REPO}"))?;
...
/// Cached model session slot for VoxCPM2 worker thread
pub struct LoadedVoxCpmModel {
    pub gpu: bool,
    pub model: pocket_tts::TTSModel,
}

pub type VoxCpmModelSlot = Option<LoadedVoxCpmModel>;
...
pub(crate) fn ensure_vox_cpm_2_loaded(
    ...
    if model.as_ref().is_none_or(|m| m.gpu != cfg.gpu) {
        ...
        info!("Loading VoxCPM2 neural speech model session in pure Rust...");
        *model = Some(LoadedVoxCpmModel {
            gpu: cfg.gpu,
            model: load_pocket_tts_model_on_gpu(POCKET_TTS_VARIANT, cfg.gpu)
                .context("load VoxCPM2 neural model")?,
        });
        info!("VoxCPM2 model loaded in {:?}", started.elapsed());
    }
    Ok(())
}
...
pub(crate) fn speak_vox_cpm_2(
    ...
    ensure_vox_cpm_2_loaded(config, model)?;
    let tts_model = &model.as_ref().unwrap().model;

    // Determine reference audio clip and transcript based on voice_mode
    let is_clone_mode = cfg.voice_mode == "clone"
        || (!cfg.cloned_voice.trim().is_empty() && cfg.voice_mode != "prompt");

    let (ref_clip_string, _transcript) = if is_clone_mode {
        let voice_id = if cfg.cloned_voice.trim().is_empty() {
            "alba"
        } else {
            cfg.cloned_voice.trim()
        };
        ...
        if cfg.ultimate_cloning {
            if let Some(ref trans) = transcript {
                info!("VoxCPM2 Ultimate Cloning active with transcript: '{}'", trans);
            } else {
                info!("VoxCPM2 Ultimate Cloning enabled; looking for companion .txt alongside '{}'", wav_path);
            }
        }
        (wav_path, transcript)
    } else {
        // Voice Design: map natural language speaker_prompt to neural acoustic reference
        let prompt_lower = cfg.speaker_prompt.to_lowercase();
        let prompt_trim = cfg.speaker_prompt.trim();

        let female_pool = [
            "hf://kyutai/tts-voices/vctk/p228_023_enhanced.wav", // Clear/High
            "hf://kyutai/tts-voices/vctk/p229_023_enhanced.wav", // Soft/Warm
            ...
            "hf://kyutai/tts-voices/alba-mackenna/casual.wav",   // Casual Female
        ];

        let male_pool = [
            "hf://kyutai/tts-voices/vctk/p254_023_enhanced.wav", // Standard Male
            "hf://kyutai/tts-voices/vctk/p360_023_enhanced.wav", // Deep/Bass Male
    ...
    if !voice_states.contains_key(ref_clip) {
        info!("Computing VoxCPM2 voice state embedding for {ref_clip}...");
        ...
        let state = tts_model
            ...
            .context("compute VoxCPM2 neural voice state")?;
        voice_states.insert(ref_clip.to_string(), state);
    }
    let voice_state = voice_states.get(ref_clip).unwrap();

    if is_prewarm {
        info!("VoxCPM2 model and voice state prewarmed instantly.");
        return Ok(());
    }

    info!(
        "Synthesizing text with VoxCPM2 in pure Rust (prompt='{}', ref_clip='{}', speed={:.2})...",
    ...
    for chunk in tts_model.generate_stream(&u.text, voice_state) {
        ...
        let chunk = chunk.context("voxcpm generate (stream)")?;
        let chunk = chunk.squeeze(0).context("squeeze voxcpm audio chunk")?;
        let bytes = pocket_tts::audio::pcm_i16_le_bytes(&chunk).context("encode voxcpm audio chunk")?;
        ...
        if !samples.is_empty() {
            sink.append(rodio::buffer::SamplesBuffer::new(1, VOX_CPM_2_SAMPLE_RATE, samples));
